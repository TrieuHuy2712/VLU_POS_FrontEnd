...
export class FilterOrderComponent implements OnInit {
  ...
  public search() {
    ...
    this.loadProducts();
    this._dataService.get('/api/POS1order/getlistpaging?page=' + this.pageIndex
  ...
  FieldsChange(e, id) {
    if (e.target.checked == true) {
      this._dataService.get('/api/POS1order/detail/' + id).subscribe((response: any) => {
        this.entity = response;
        console.log(response);
  ...
  public delete(id: string) {
    this.notificationService.printConfirmationDialog(MessageConstants.CONFIRM_DELETE_MSG, () => {
      this._dataService.delete('/api/POS1order/delete', 'id', id).subscribe((response: any) => {
  ...
  public saveData() {
    ...
    for (let i = 0; i < this.backupItem.length; i++) {
      ...
      this._dataService.put('/api/POS1order/update', this.backupItem[i]).subscribe((response: any) => {
        this.search();
        console.log(response);
      });
